...
// Generate values with 1 significant figure: 1, 2, 3, ..., 9, 10, 20, 30, ..., 90, 100, 200, etc.
...
// Format value in scientific notation (e.g., "2×10⁻³")
...
// Gamma slider class (1e-3 to 1)
class GammaSlider {
  constructor() {
    this.values = generateOneSigFigValues(0.001, 1);
  }
...
const gammaSlider = new GammaSlider();

// Render gamma sliders based on current d
function renderGammaSliders() {
  const container = document.getElementById('gammaSliders');
  container.innerHTML = '';
  ...
  for (let i = 0; i < currentD; i++) {
    ...
    slider.min = '0';
    slider.max = '100';
    slider.value = gammaSlider.valueToSlider(gammaValues[i]).toString();
    slider.style.width = '100px';
    slider.dataset.index = i;

    const valueDisplay = document.createElement('span');
    valueDisplay.innerHTML = formatScientific(gammaValues[i]);
    valueDisplay.style.cssText = 'min-width: 60px; text-align: left; font-size: 13px;';
    valueDisplay.id = `gammaValue${i}`;

    slider.addEventListener('input', (e) => {
      const idx = parseInt(e.target.dataset.index);
      const value = gammaSlider.sliderToValue(parseFloat(e.target.value));
      gammaValues[idx] = value;
      appState.gammaValues[idx] = value;
      appState.save();
      document.getElementById(`gammaValue${idx}`).innerHTML = formatScientific(value);
    });
  ...
  // Render MathJax for gamma labels
...
// Logscale slider for hidden dimension k
// Range: 10 to 30000, default 1000
class HiddenDimSlider {
  constructor(min, max) {
...
const kSlider = new HiddenDimSlider(10, 30000);

// Learning rate slider class (1e-4 to 1)
class LearningRateSlider {
  constructor() {
    this.values = generateOneSigFigValues(0.0001, 1);
  }
...
const etaSlider = new LearningRateSlider();

// Batch size slider class (1 to 1000)
class BatchSizeSlider {
  constructor() {
    this.values = generateOneSigFigValues(1, 1000);
  }
...
const batchSizeSlider = new BatchSizeSlider();
...
// Render alpha sliders based on current d
function renderAlphaSliders() {
  const container = document.getElementById('alphaSliders');
  container.innerHTML = '';
  ...
  for (let i = 0; i < currentD; i++) {
    ...
    slider.min = '0';
    slider.max = '3';
    slider.value = alphaValues[i].toString();
    slider.style.width = '100px';
    slider.dataset.index = i;

    const valueDisplay = document.createElement('span');
    valueDisplay.textContent = alphaValues[i].toString();
    valueDisplay.style.cssText = 'min-width: 30px; text-align: left; font-size: 13px;';
    valueDisplay.id = `alphaValue${i}`;

    slider.addEventListener('input', (e) => {
      const idx = parseInt(e.target.dataset.index);
      const value = parseInt(e.target.value);
      alphaValues[idx] = value;
      appState.alphaValues[idx] = value;
      appState.save();
      document.getElementById(`alphaValue${idx}`).textContent = value.toString();
      updateTargetFunctionEquation();
    });
  ...
  // Render MathJax for alpha labels
  if (window.MathJax && window.MathJax.typesetPromise) {
    MathJax.typesetPromise([container]).catch((err) => console.log(err));
  } else if (window.MathJax && window.MathJax.Hub) {
    MathJax.Hub.Queue(["Typeset", MathJax.Hub, container]);
...
function renderNetworkViz() {
  ...
  const width = 400;
  const gap = 6; // Gap between trapezoids and lines (2x)
  ...
  const calcHeight = (dim) => 20 * (Math.log2(dim) + 1);
  ...
  const height = maxHeight + 2 * padding + 50; // 50px for labels above/below
