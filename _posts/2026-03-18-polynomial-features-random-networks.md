---
layout: post
title: "How to see polynomial features hidden in random neural networks"
date: 2026-03-18 07:00:00
category: kernels
emoji: 🌈
---

The language of polynomials is beautiful and well-developed: centuries of work have given us a rich, deep theory that spans analysis and algebra and gives powerful tools for the study of multivariate polynomials. The language of neural network features has not yet been written, but it promises to be beautiful: empirics show that hidden features can represent complex concepts and form striking circuitry. Fortunately, more and more results in fundamental deep learning theory suggest that the language of polynomials might actually be the language of hidden features. How wonderful if we could fathom the depths of learning and cognition with polynomial algebra!

My (biased) favorite example of this comes from [my recent paper](https://arxiv.org/abs/2510.14878), which will appear at ICLR in April. We took a careful look at the inductive biases of simple learning methods and found that (a) they treat real image datasets as Gaussian blobs and (b) learning on these blobs decomposes into independent learning of Hermite polynomial coefficients. What falls out is a surprisingly simple and beautiful theory of inductive bias which makes excellent (i.e. correct) predictions of test-time performance on different tasks. Just look at these curves:
...
You might be wondering: is there a simpler way to see how this polynomial language comes about? In fact, there is: the purpose of this note is to walk through the deep underlying reason why Hermite polynomials arise in the description of neural network hidden features.
...
The key idea is that these secretly *are* polynomial features *up to rotation.* After a suitable orthogonal transformation on $\mathbf{h}(\mathbf{x})$, we get (nearly-)polynomial features. This is most easily seen with kernel language: take a second data vector $\mathbf{x}'$ and consider the feature kernel $K(\mathbf{x}, \mathbf{x}') := \frac{1}{n} \langle \mathbf{h}(\mathbf{x}), \mathbf{h}(\mathbf{x}') \rangle$. Let us make the following assumption on this kernel:

<div style="border: 1px solid #aaa; border-radius: 4px; padding: 1em 1.5em; margin: 1.5em 0;">
<strong>Assumption A (dot-product feature kernel).</strong> The feature kernel admits the following form:
...
The problem with Assumption A is that, strictly speaking, it's false. Random feature kernels do not take exact dot-product form, even in the infinite-width limit. Fortunately, we can often make that assumption, and it's not too wrong.
...
(This is precisely the neural network Gaussian process (NNGP) kernel of [Lee et al. (2017)](https://arxiv.org/abs/1711.00165).) Remember that we're initializing our random features as $W_{ij} \sim \mathcal{N}(0, \frac{\alpha}{d})$, so that's where the $\frac{\alpha}{d}$ comes in.
...
However, it is *not* a true dot product kernel; those first two norm arguments are real. That said, we can ignore the first two arguments if we know they do not vary over our dataset. For example, if we know that every data vector we will care about lies on a sphere: $\|\mathbf{x}_i\| = r \ \forall i$, then we can drop the first two arguments, and we truly get a dot-product kernel.

Of course, it's a rare case when our data's exactly on a sphere, but it's often the case that the data roughly concentrates in norm, as is the expectation in high dimension. In such cases, we *approximately* get a dot-product kernel, and the eigensystem is *approximately* built from Hermite polynomials. In practice, this approximation is actually really good: in our paper, we find that assuming concentration of norm allows us to predict the eigensystem of CIFAR-10 with great accuracy. This is especially surprising given that the data norms don't really concentrate very tightly:
...
If I'm honest, I don't really know why we didn't have to take corrections involving the width of this distribution into account. It's probably something that some future theory paper will have to go back and understand. A practical takeaway is that this assumption of concentration of norm of data vectors actually bears a lot of weight, probably more than you'd think it would. As a result, we can actually take Assumption A for real image datasets, which gives us a dot-product kernel, polynomial features, and a Hermite eigensystem.
...
where $h_\ell(z) = \frac{1}{\sqrt{\ell!}} \text{He}_\ell(z)$ is the normalized probabilist's Hermite polynomial. In words, we're extracting the $\ell$-th Hermite coefficient of the function $z \mapsto \sigma\left( r\sqrt{\tfrac{\alpha}{d}}\, \cdot z \right)$. That prefactor of $r\sqrt{\tfrac{\alpha}{d}}$ makes a lot of sense: it's the standard deviation of the (Gaussian!) preactivations fed into the activation function $\sigma(\cdot)$.[^hermite-history]

This is a powerful result: if you know the Hermite decomposition of $\sigma(\cdot)$, then you know the level coefficients of your kernel and the relative weights of the different orders of monomial features. The Hermite decompositions of common activation functions (e.g. $\text{ReLU}$) can be worked out in closed form. Even simpler, a lot can be learned from symmetries and taking note of which Hermite coefficients are exactly zero. For example, $\sigma = \tanh$ is odd, so it will lack all even coefficients: $c_{2\ell} = 0$. In a weirder case, $\sigma = \text{ReLU}$ equals a linear function plus an even function, so it has $c_\ell = 0$ for $\ell = 3, 5, 7, \cdots.$ This particular pathology of $\text{ReLU}$ has shown up [so many times]({{site.baseurl}}/blog/the-expressivity-of-shallow-relu-nets/) in my research.

## Activation function $\rightarrow$ kernel coefficients near the origin

Now things are going to get somewhat subtle. So far, when we've talked about concentration of norm, we've really meant that the norm of the data vectors doesn't vary much in a *relative* sense, not in an absolute sense. That is, we care that

$$
\frac{\max_i \|\mathbf{x}_i\|}{\min_j \|\mathbf{x}_j\|} \approx 1, \qquad \text{not that} \qquad \max_i \|\mathbf{x}_i\| - \min_j \|\mathbf{x}_j\| \approx 0.
$$

However, there *is* a very important solvable limit in which the data vectors concentrate in norm absolutely but not relatively. This is the limit in which the data vectors all have small norm: $\|\mathbf{x}_i\| \ll 1$. If all the vectors have the *same* small norm (e.g., $\|\mathbf{x}_i\| = \|\mathbf{x}_j\| = r \ll 1$), or the variation is small in a relative sense, then we recover the case above. However, we want an analysis valid for all $\mathbf{x} \in r B^{d}$, where $r \ll 1$ and $B^d$ is the $d$-ball.

The method of attack here is direct: simply examine the kernel $K_{\sigma,\alpha}(\|\mathbf{x}\|^2, \|\mathbf{x}'\|^2, \langle\mathbf{x},\mathbf{x}'\rangle)$ and naively (Taylor-)expand it for small $\|\mathbf{x}\|^2, \|\mathbf{x}'\|^2$. Upon trying this, one quickly makes a few observations:

- It is not always possible to take this expansion analytically. For example, the Laplace kernel $e^{-\|\mathbf{x} - \mathbf{x}'\|}$ is not smooth around zero.
...
In the above, $\sigma^{(k)}(0)$ denotes the $k$-th derivative of $\sigma$ evaluated at zero. When the norms are small ($\|\mathbf{x}_i\|, \|\mathbf{x}_j\| \ll 1$) the $q, q' = 0$ terms dominate those with $q, q' \ge 1$, and we can heuristically approximate
...
This is a main result in what I think of as "wide kernel theory": the study of kernels which are much wider than the data distributions on which they're evaluated, often leading to great simplifications and nice polynomial forms.
