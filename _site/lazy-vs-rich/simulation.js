...
export class Simulation {
  ...
  reset() {
    ...
    this.W0 = null; // student init snapshot: n x 2 (frozen, for centering)
    this.a0 = null; // student init snapshot: n (frozen, for centering)
  ...
  initialize(params) {
    ...
    // Student weights — fully independent init.
    // Centering trick: store W0, a0 at init; training target is f*(x) - f_0(x).
    this.W = Array.from({ length: n }, () => [rng.randn(), rng.randn()]);
    this.a = Array.from({ length: n }, () => rng.randn());

    // Frozen init snapshot for centering
    this.W0 = this.W.map(w => [w[0], w[1]]);
    this.a0 = this.a.slice();
  ...
  // Forward pass using frozen init weights — for centering trick
  initForward(x) {
    const { n, alpha } = this.params;
    const scale = alpha / n;
    let out = 0;
    for (let i = 0; i < n; i++) {
      const z = this.W0[i][0] * x[0] + this.W0[i][1] * x[1];
      out += this.a0[i] * relu(z);
    }
    return scale * out;
  }
  ...
  step(eta, batchSize) {
    ...
    for (const x of batch) {
      ...
      const pred   = this.studentForward(x) - this.initForward(x); // centered
