...
const RED_DOT    = 'rgba(40, 40, 40, 0.85)';
const BLUE_DOT   = 'rgba(40, 40, 40, 0.85)';
const RED_TEACHER  = 'rgba(40, 40, 40, 0.45)';
const BLUE_TEACHER = 'rgba(40, 40, 40, 0.45)';
const RED_TRACE    = 'rgba(40, 40, 40, 0.2)';
const BLUE_TRACE   = 'rgba(40, 40, 40, 0.2)';
...
// Plugin: draw gridlines every GRID_STEP units on both axes, independent of ticks.
const GRID_STEP = 2;
const fixedGridPlugin = {
  ...
  afterDraw(chart) {
    ...
    // Vertical lines (x gridlines)
    const xStart = Math.ceil(xScale.min / GRID_STEP) * GRID_STEP;
    for (let v = xStart; v <= xScale.max + 1e-9; v += GRID_STEP) {
    ...
    // Horizontal lines (y gridlines)
    const yStart = Math.ceil(yScale.min / GRID_STEP) * GRID_STEP;
    for (let v = yStart; v <= yScale.max + 1e-9; v += GRID_STEP) {
...
export class WeightPlot {
  constructor(canvasId) {
    ...
    this.chart = new Chart(ctx, {
      type: 'scatter',
      data: { datasets: [] },
      plugins: [niceTick2DPlugin, fixedGridPlugin],
      options: {
        animation: false,
        responsive: true,
        ...
        scales: {
          x: {
            ...
            title: { display: true, text: 'W[:,1]', font: { size: 17, family: MONO } },
            grid: { display: false },
            ticks: { font: { size: 16, family: MONO } },
          },
          y: {
            type: 'linear', min: -3, max: 3,
            title: { display: true, text: 'W[:,2]', font: { size: 17, family: MONO } },
            grid: { display: false },
            ticks: { font: { size: 16, family: MONO } },
          },
        },
  ...
  setFontScale(s) {
    this.chart.options.scales.x.title.font.size = Math.round(17 * s);
    this.chart.options.scales.y.title.font.size = Math.round(17 * s);
    this.chart.options.scales.x.ticks.font.size = Math.round(16 * s);
    this.chart.options.scales.y.ticks.font.size = Math.round(16 * s);
    this.chart.options.plugins.legend.labels.font.size = Math.round(12 * s);
    this.chart.update('none');
  }
