...
fontScaleSlider.addEventListener('input', () => {
  const s = parseFloat(fontScaleSlider.value) / 100;
  fontScaleValue.textContent = s.toFixed(1) + '×';
  weightPlot.setFontScale(s);
  lossChart.setFontScale(s);
});

// Alpha slider — update display and reset if simulation has already run
alphaSlider.addEventListener('input', () => {
  updateAlpha();
  // If simulation has already run (not in initial state), reset it
  if (sim.iteration > 0) {
    preSample();
  }
});
